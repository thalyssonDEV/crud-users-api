...
@app.route('/usuarios/web', methods=['POST'])
def add_info():
    # Captura os dados do formulário enviados via POST
    nome = request.form.get('nome') # Captura o nome do input
    idade = request.form.get('idade')  # Captura a idade do input
    
    # Chama a função no script.py para processar os dados
    novo_usuario = script.processar_usuario(nome, idade)
    
    return jsonify(novo_usuario), 201  # Retorna o novo usuário em formato JSON
...
def delete_usuario(id):
    ...
    dados.remove(usuario)
    salvar_dados(dados)
    return jsonify({"DELETE": "Usuário deletado com sucesso"})
